...
def load_latency_dict(dataset, channels, kernel_size, gpu):
  results = dict()
  for baseline in BASELINES:
    path = f"results/{gpu}/{DATASETS[dataset]}-bs=1-c={CHANNELS[channels]}-ks={kernel_size}.{BASELINES[baseline]}.json"
    with open(path, "r") as reader:
      results[baseline] = np.mean(json.load(reader)['latency_full'])
  return results


def plot_gpu(args, gpu: str):
  figure: pyplot.Figure = pyplot.figure(figsize=(5, 2.0), dpi=224)
  ...
  for i, channels in enumerate(CHANNELS):
    ...
    channel_values = normalize(channel_values)

    chart.set_group(group_name, channel_values)
  ...
  chart.xaxis.ticks.font_dict.update({'size': 10})
  chart.xaxis.ticks.rotation = 20
  ...
  filename = f"figure10_layer_wise_speedup.{gpu}"
