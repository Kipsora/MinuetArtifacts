__all__ = [
    ...
    'query_sorted_index_with_offsets_dynamic_block_size',
]
...
def query_sorted_index_with_offsets_dynamic_block_size(
    sources: torch.Tensor, targets: torch.Tensor, offsets: torch.Tensor,
    thread_block_size: int, source_block_size: int, target_block_size: int):
  sources = sources.contiguous()
  targets = targets.contiguous()
  offsets = offsets.contiguous()
  if offsets.is_cuda:
    return _C.cuda_query_sorted_index_with_offsets_dynamic_block_size(
        thread_block_size, source_block_size, target_block_size, sources,
        targets, offsets)
  raise NotImplementedError
