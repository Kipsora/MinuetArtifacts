...
torch::Tensor CUDAQuerySortedIndexWithOffsetsDynamicBlockSize(
    std::size_t thread_block_size, std::size_t source_block_size,
    std::size_t target_block_size, const torch::Tensor &sources,
    const torch::Tensor &targets, const torch::Tensor &offsets) {
  MINUET_ENSURE_TENSOR_NDIM(sources, 2);
  MINUET_ENSURE_TENSOR_NDIM(targets, 2);
  MINUET_ENSURE_TENSOR_NDIM(offsets, 2);
  ...
  auto device = GetTorchDeviceFromTensors({sources, targets, offsets});
  auto context = GetCUDAContextFromTorchCUDADevice(device);
  auto ctype = GetTorchScalarTypeFromTensors({sources, targets, offsets});
  auto itype = ctype;
  auto indices = torch::empty({num_offsets, num_targets},
  ...
  do {                                                                      \
    if (ctype == torch::CppTypeToScalarType<CT>::value &&                   \
      ...
      cuda::QuerySortedIndexWithOffsets().operator()<CT, IT, T_NDIM>(       \
          num_sources, num_targets, num_offsets, sources.data_ptr<CT>(),    \
          targets.data_ptr<CT>(), offsets.data_ptr<CT>(),                   \
          indices.data_ptr<IT>(), thread_block_size, source_block_size,     \
          target_block_size, context);                                      \
      return indices;                                                       \
    }                                                                       \
  } while (false)
  MINUET_FOR_ALL_DIMS_AND_CI_TYPES(CASE);
...
MINUET_TORCH_REGISTER(cuda_query_sorted_index_with_offsets_dynamic_block_size,
                      CUDAQuerySortedIndexWithOffsetsDynamicBlockSize);
