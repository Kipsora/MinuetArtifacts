...
template <typename CT, typename IT, std::size_t T_NDIM,
          std::size_t T_THREAD_BLOCK_SIZE, std::size_t T_SOURCE_BLOCK_SIZE,
          std::size_t T_TARGET_BLOCK_SIZE>
void QuerySortedIndexWithOffsetsDynamicSize(std::size_t num_sources,  //
                                            std::size_t num_targets,  //
                                            std::size_t num_offsets,  //
                                            const CT *d_sources,      //
                                            const CT *d_targets,      //
  ...
  static_assert(T_TARGET_BLOCK_SIZE >= T_THREAD_BLOCK_SIZE);
  static_assert(T_TARGET_BLOCK_SIZE % T_THREAD_BLOCK_SIZE == 0);
  ...
  const auto max_offsets_per_round =
      std::min(DivFloor(MAX_SHARED_MEMORY_SIZE, OFFSET_SIZE), num_offsets);
  const auto shared_memory_size = std::max(max_offsets_per_round * OFFSET_SIZE,
                                           sizeof(IT) * T_THREAD_BLOCK_SIZE);

  const auto num_source_blocks = DivCeil(num_sources, T_SOURCE_BLOCK_SIZE);
  auto d_merged =
      context.NewBuffer<IT>(2 * num_source_blocks * num_offsets + 1);
  auto d_borders = d_merged.device_data();
  auto d_target_cumsum_blocks =
  ...
  auto NumBlocks = [num_source_blocks, num_targets, num_offsets,
    ...
    auto n =
        (b + 1 < num_source_blocks) ? borders[x + num_offsets] : num_targets;
    return DivCeil(n - borders[x], T_TARGET_BLOCK_SIZE);
  };
  CUB_TRANSFORMED_INPUT_ITERATOR(UIter, NumBlocks, d_in);

  context.Launch(
      num_source_blocks * max_offsets_per_round, T_THREAD_BLOCK_SIZE,
      shared_memory_size,
      kernels::ComputeTargetBorders<CT, IT, T_NDIM, T_SOURCE_BLOCK_SIZE>,
      num_source_blocks,   //
      num_targets,         //
      num_offsets,         //
      shared_memory_size,  //
  ...
  MINUET_CHECK_CUDA(cudaMemsetAsync(d_target_block_indices.device_data(), 0,
                                    sizeof(IT) * num_target_blocks,
                                    context.stream()));
  context.Launch(num_source_blocks * num_offsets, T_THREAD_BLOCK_SIZE, 0,
                 kernels::FillBlockIndices<IT, IT>,
                 num_source_blocks * num_offsets,  //
                 d_target_cumsum_blocks,           //
                 d_target_block_indices.device_data());
  Materialize<T_THREAD_BLOCK_SIZE>(
      num_source_blocks * num_offsets, num_target_blocks,
      d_target_cumsum_blocks, d_target_block_indices.device_data(), context);
  context.Launch(
      DivCeil<std::size_t>(num_target_blocks, 8) * T_THREAD_BLOCK_SIZE,
      T_THREAD_BLOCK_SIZE, 0,
      kernels::QuerySortedIndexWithOffsets<CT, IT, T_NDIM, T_SOURCE_BLOCK_SIZE,
                                           T_TARGET_BLOCK_SIZE>,
      num_sources,             //
      num_targets,             //
      num_source_blocks,       //
...
void QuerySortedIndexWithOffsets::operator()(std::size_t num_sources,        //
                                             ...
                                             std::size_t source_block_size,  //
                                             std::size_t target_block_size,  //
                                             const Context &context) const {
#define CASE(THREAD_BLOCK_SIZE, SOURCE_BLOCK_SIZE, TARGET_BLOCK_SIZE)          \
  do {                                                                         \
    if (thread_block_size == THREAD_BLOCK_SIZE &&                              \
        source_block_size == SOURCE_BLOCK_SIZE &&                              \
        target_block_size == TARGET_BLOCK_SIZE) {                              \
      QuerySortedIndexWithOffsetsDynamicSize<                                  \
          CT, IT, T_NDIM, THREAD_BLOCK_SIZE, SOURCE_BLOCK_SIZE,                \
          TARGET_BLOCK_SIZE>(num_sources, num_targets, num_offsets, d_sources, \
                             d_targets, d_offsets, d_indices, context);        \
      return;                                                                  \
    }                                                                          \
  } while (false)
  CASE(128, 32, 1024);
  ...
  MINUET_ERROR("Unimplemented configuration thread_block_size=",
               thread_block_size, " source_block_size=", source_block_size,
               " target_block_size=", target_block_size);
#undef CASE
}
...
#define MINUET_EXPLICIT_INSTANTIATOR(_, T_NDIM, CT, IT)                       \
  ...
  template void QuerySortedIndexWithOffsets::operator()<CT, IT, T_NDIM>(      \
      std::size_t, std::size_t, std::size_t, const CT *, const CT *,          \
      const CT *, IT *, std::size_t, std::size_t, std::size_t,                \
      const Context &) const
